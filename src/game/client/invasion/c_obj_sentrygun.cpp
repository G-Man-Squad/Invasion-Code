...
void C_ObjectSentrygun::GetBoneControllers(float controllers[MAXSTUDIOBONECTRLS])
{
	...
	if (!IsPreviewingYaw())
	{
		Studio_SetController( GetModelPtr(), 0, m_fBoneXRotator, controllers[0] );
	}
	else
	{
		// Bone rotation == 0 here to make it exactly match the preview
		Studio_SetController( GetModelPtr(), 0, 0, controllers[0] );
	}

	Studio_SetController( GetModelPtr(), 1, m_fBoneYRotator, controllers[1] );
	Studio_SetController( GetModelPtr(), 2, m_fBoneYRotator, controllers[2] );
	Studio_SetController( GetModelPtr(), 3, m_fBoneYRotator, controllers[3] );
}
