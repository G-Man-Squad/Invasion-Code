...
{
	BaseClass::GetVehicleViewPosition( nRole, pOrigin, pAngles, NULL );
	return;
...
void C_ObjectBaseMannedGun::GetBoneControllers(float controllers[MAXSTUDIOBONECTRLS], float dadt)
{
	...
	Studio_SetController( GetModelPtr(), 0, m_flGunYaw,   controllers[0] );
	Studio_SetController( GetModelPtr(), 1, m_flGunPitch, controllers[1] );

	if ( m_nMoveStyle == MOVEMENT_STYLE_BARREL_PIVOT )
	{
		Studio_SetController( GetModelPtr(), 2, m_flBarrelPitch, controllers[2] );
	}
}
