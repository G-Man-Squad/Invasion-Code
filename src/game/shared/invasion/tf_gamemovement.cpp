...
void CTFGameMovement::CategorizePosition( void )
{
	...
	trace_t trace;
	Vector vStart( mv->GetAbsOrigin().x, mv->GetAbsOrigin().y, mv->GetAbsOrigin().z );
...
//				VectorCopy( trace.endpos, mv->GetAbsOrigin() );
			}		
		}
...
bool CTFGameMovement::CheckJumpButton( void )
{
	...
	PlayStepSound( player->m_pSurfaceData, 1.0, true );
