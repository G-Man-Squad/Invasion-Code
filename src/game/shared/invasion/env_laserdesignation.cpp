...
bool CEnvLaserDesignation::GetLaserDesignation( int iTeamNumber, int iDesignator, Vector *vecOrigin )
{
	...
	CEnvLaserDesignation* pLaser;
	if ( iTeamNumber == 1 )
	{
		Assert( iDesignator < m_LaserDesignatorsTeam1.Count() );
		pLaser = dynamic_cast<CEnvLaserDesignation*>( m_LaserDesignatorsTeam1[iDesignator].Get() );
	}
	else
	{
		Assert( iDesignator < m_LaserDesignatorsTeam2.Count() );
		pLaser = dynamic_cast<CEnvLaserDesignation*>( m_LaserDesignatorsTeam2[iDesignator].Get() );
	}

	// Active?
	if ( !pLaser || !pLaser->IsActive() )
		return false;

	*vecOrigin = pLaser->GetAbsOrigin();
	return true;
}
