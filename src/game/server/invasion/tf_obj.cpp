...
//-----------------------------------------------------------------------------
bool CBaseObject::ClientCommand( CBaseTFPlayer *pSender, const CCommand &pCmd )
{
	if ( FStrEq( pCmd[0], "dismantle"))
	{
		DismantleCommand( pSender );
		return true;
	}

	if ( FStrEq( pCmd[0], "yaw" ) )
	{
		if ( pCmd.ArgC() == 2 )
		{
			float flYaw = atof( pCmd[1] );
			YawCommand( pSender, flYaw );
		}
		return true;
	}

	if ( FStrEq( pCmd[0], "takecontrol" ) )
	{
...
{ 
	...
	// If I'm being built, only allow +use if I don't have a sapper on me
