...
bool CVehicleMortar::ClientCommand( CBaseTFPlayer *pPlayer, const CCommand &args )
{
	...
	if ( !Q_stricmp( args[0], "Deploy"))
	{
		Deploy();
		return true;
	}
	else if ( !Q_stricmp( args[0], "Undeploy" ) )
	{
		UnDeploy();
	}
	else if ( !Q_stricmp( args[0], "CancelDeploy" ) )
	{
		CancelDeploy();
		return true;
	}
	else if ( !Q_stricmp( args[0], "FireMortar" ) )
	{
		if ( args.ArgC() == 3 )
		{
	...
	else if ( !Q_stricmp( args[0], "MortarYaw" ) )
	{
		if ( args.ArgC() == 2 )
		{
