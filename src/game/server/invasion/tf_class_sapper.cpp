...
BEGIN_SEND_TABLE_NOBASE( CPlayerClassSapper, DT_PlayerClassSapperData )
	SendPropFloat	( SENDINFO_STRUCTELEM( m_ClassData.m_flDrainedEnergy ), 32, SPROP_NOSCALE ),
END_SEND_TABLE()
...
void CPlayerClassSapper::RespawnClass( void )
{
	...
	m_ClassData.m_flDrainedEnergy = 0;
...
void CPlayerClassSapper::AddDrainedEnergy( float flEnergy )
{
	...
	m_ClassData.m_flDrainedEnergy = MIN( 1.0, m_ClassData.m_flDrainedEnergy + flEnergy );

	// Did we hit max?
	if ( m_ClassData.m_flDrainedEnergy >= 1.0 )
	{
		m_ClassData.m_flDrainedEnergy = 0;
...
{
	return m_ClassData.m_flDrainedEnergy;
}
...
{
	m_ClassData.m_flDrainedEnergy = MAX( 0, m_ClassData.m_flDrainedEnergy - flEnergy );
}
